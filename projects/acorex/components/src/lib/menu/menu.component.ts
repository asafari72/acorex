...
import { AXPlacement, AXMenuItem, AXHtmlUtil, AXIPoint, AXBaseSizableComponent, AXElementSize } from '@acorex/core';
...
export class AXMenuComponent implements AXBaseSizableComponent {
  ...
  private fixItemMap(items: AXMenuItem[]) {
    ...
    items.forEach((item) => {
      (item as any).hasChildren = item.items && item.items.filter((c) => c.visible !== false).length > 0;
    ...
    this.zone.runOutsideAngular(() => {
      ...
      if (ul) {
        ...
        if (ul.classList.contains('collapsed')) {
          ...
          const posLi = li.getBoundingClientRect();
          let y = 0;
          let x = 0;
          ...
          if (
            ...
            const parentPost = ul.parentElement.getBoundingClientRect();
            if (ul.classList.contains('first')) {
  ...
  applyResponsive() {
    setTimeout(() => {
      ...
      const moreUiEl = this.moreUL.nativeElement;
      const moreLiEl = this.moreLI.nativeElement;
      let liArray = [];
      this.zone.runOutsideAngular(() => {
        liArray = [].slice
        ...
        if (containerEl.clientWidth < rootEl.scrollWidth) {
          ...
          liArray = [].slice.call(moreUiEl.querySelectorAll('li')).filter((c) => c.parentNode === moreUiEl) as Array<HTMLLIElement>;
          let sum = 0;
          liArray.forEach((li) => {
  ...
  applyContextMenu() {
    if (this.target && this.mode !== 'visible') {
      this.zone.runOutsideAngular(() => {
        const root = this.container.nativeElement as HTMLElement;
        if (!root.classList.contains('contextMenu')) {
          root.classList.add('contextMenu');
        }
        if (this.mode !== 'manual') {
          const eventType: string = this.mode === 'click' ? 'click' : 'contextmenu';
